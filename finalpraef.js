...
function removeNonMatchingElements(sublists) {
...
function processAndMoveMatchingElements(list1, list2, g) {
  if (list1.length > g) {
...
function findMostFrequentMatchingElement(list1, list2, g) {
...
function findMostFrequentHalbMatchingElement(list1, list2, g) {
  let mostFrequentElement = null;
  let maxFrequency = 0;
...
function addElementsToList(list1, list2, g) {
  while (list1.length < g && list2.length > 0) {
...
function elementsToListOfLists(elements) {
...
function groupSizes (susListe, g) {
    const gruppenGroe = [];
...
function groupCount (liste, groupSize) {
...
function countMatches(liste1, liste2, maxSize) {
...
function findOuterListWithMaxMatches(matchesByOuterList) {
...
function extractSublistByKey(listOfLists, keyValue) {
...
function insertValueIntoSublistAndRemove(list, keyValue, otherList) {
  const [key, value] = Object.entries(keyValue)[0]; // Extrahiere den Key und den Value
...
function countHalbMatches(liste1, liste2, maxSize) {
  const matchesByOuterList = {};
  let maxGroesse = maxSize;
  for (let i = 0; i < liste1.length; i++) {
    const outerList1 = liste1[i];
...
function countViertelMatches(liste1, liste2, maxSize) {
  const matchesByOuterList = {};
  let maxGroesse = maxSize;
  for (let i = 0; i < liste1.length; i++) {
    const outerList1 = liste1[i];
...
function findElementWithMostMatches(liste) {
...
function countMatchesForElement(element, liste) {
  let matchesCount = 0;
...
function findElementWithMostHalbMatches(liste) {
  let elementWithMostMatches = null;
  let mostMatches = -Infinity; // Anfangswert auf -Infinity setzen
...
function countHalbMatchesForElement(element, liste) {
  let matchesCount = 0;
...
function insertElementIntoSmallestSublistAndRemove(element, listOfLists, anotherList) {
...
function lastTotalCount(pushList) {
    let halbMatchcount = 0;
    for (const sublist of pushList) {
...
const shuffledList = shuffleArray(susListe);

const gruppenGroessen = groupSizes(shuffledList, g);  //   Objekt mit maximaler Gruppengroesse erstellen 

const nonMatchingElements = removeNonMatchingElements(shuffledList);  // Elemente fürn die erste Reihe

processAndMoveMatchingElements(nonMatchingElements, shuffledList, g); // falls erste Reihe größer als g

findMostFrequentMatchingElement(nonMatchingElements, shuffledList, g); // falls g kleiner

findMostFrequentHalbMatchingElement(nonMatchingElements, shuffledList, g);

addElementsToList(nonMatchingElements, shuffledList,g)

const firstrow = elementsToListOfLists(nonMatchingElements); // Erste Reihe von Elementen einsetzen

let push;

while (true) {
let maxGruppe = groupCount (firstrow, gruppenGroessen);
let counts = countMatches(firstrow, shuffledList, maxGruppe);

let isEmpty = Object.keys(counts).every(key => Object.keys(counts[key]).length === 0);

if (isEmpty) {
    maxGruppe = groupCount (firstrow, gruppenGroessen);
    counts = countHalbMatches (firstrow, shuffledList, maxGruppe);
    isEmpty = Object.keys(counts).every(key => Object.keys(counts[key]).length === 0);
  }

if (isEmpty) {
    maxGruppe = groupCount (firstrow, gruppenGroessen);
    counts = countViertelMatches (firstrow, shuffledList);
    isEmpty = Object.keys(counts).every(key => Object.keys(counts[key]).length === 0);
  }
...
const bestGroup = findOuterListWithMaxMatches(counts);

const instruction = extractSublistByKey(shuffledList, bestGroup);

push = insertValueIntoSublistAndRemove(firstrow, instruction, shuffledList);
}

while (shuffledList.length > 0) {

let lastElement = findElementWithMostMatches (shuffledList);

if (lastElement != null){
  insertElementIntoSmallestSublistAndRemove (lastElement, firstrow, shuffledList);
  
  }
else {
  lastElement = findElementWithMostHalbMatches(shuffledList);
  insertElementIntoSmallestSublistAndRemove (lastElement, firstrow, shuffledList);
}
}


let finalCount = lastCount(push);

let finalTotalCount = lastTotalCount(push);

if (finalCount > finalScore || finalCount == finalScore && finalTotalCount > finalTotalScore) {
        finalScore = finalCount;
